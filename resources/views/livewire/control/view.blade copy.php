    ...
    <div class="card mb-2 shadow-sm border-start border-4 {{ $claseBorde }}">
        <div class="card-body p-2">
            <div class="d-flex justify-content-between align-items-center mb-2">
                ...
                <span class="badge {{ $item->estatus == 'pendiente' ? 'bg-warning text-dark' : ($item->estatus == 'proceso' ? 'bg-primary' : ($item->estatus == 'terminado' ? 'bg-success' : 'bg-dark')) }}">
                    {{ ucfirst($item->estatus) }}
                </span>
                <span class="text-muted small"><i class="bi bi-clock"></i> {{ \Carbon\Carbon::parse($item->fechaSol)->format('d/m H:i') }}</span>
            </div>

            <div class="row g-2 mb-2">
                <div class="col-6 small">
                    <span class="text-muted d-block" style="font-size: 0.65rem;">UBICACIÓN</span>
                    <i class="bi bi-house"></i> <strong>{{ $item->cuarto->casa->casa ?? 'N/A' }}-{{ $item->cuarto->cuarto ?? 'N/A' }}</strong>
                </div>
                <div class="col-6 small">
                    <span class="text-muted d-block" style="font-size: 0.65rem;">PRIORIDAD</span>
                    <span class="badge bg-light text-dark border">{{ $prioridads[$item->IdPrioridad] ?? 'N/A' }}</span>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div class="small text-muted">
                    <i class="bi bi-person"></i> {{ $item->tecnico->tecnico ?? 'Sin asignar' }}
                </div>
                <div class="d-flex gap-1">
                    ...
                    <button wire:click="destroy({{ $item->id }})" class="bot botRojo botChico" 
